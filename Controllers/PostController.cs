...
namespace BlogBackend.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        [Route("")]
        [AllowAnonymous]
        public async Task<IActionResult> GetAllPosts([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10, [FromQuery] int? categoryId = null)
        {
            try
            {
                var posts = await _postService.GetAllPostsAsync(pageNumber, pageSize, categoryId);
                return Ok(posts);
            }
            catch (Exception ex)
            {
